...
module.exports = {
    ...
    async login(req, res) {
        ...
        Usuario.findOne({ email_usuario: email, tipo_usuario: 1 }, function(err, user) {
            if (err) {
                console.log(err);
                res.status(200).json({ erro: "Houve um erro no servidor. Por favor, tente novamente" });
            } else if (!user) {
                ...
                user.isCorrectPassword(senha, async function(err, same) {
                    if (err) {
                        ...
                        res.status(200).jason({ status: 1, auth: true, token: token, id_client: user._id, user_name: user.nome_usuario });
                    }
                })
